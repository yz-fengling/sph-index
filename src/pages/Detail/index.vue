    ...
    <section class="con">
      ...
      <div class="conPoin">
        <div class="conPoin">
          ...
          <router-link :to="`/search/${categoryView.category1Name}`">{{categoryView.category1Name}}</router-link>
          <span>&nbsp;&nbsp;>&nbsp;&nbsp;</span>
          <router-link :to="`/search/${categoryView.category2Name}`">{{categoryView.category2Name}}</router-link>
          <span>&nbsp;&nbsp;>&nbsp;&nbsp;</span>
          <router-link :to="`/search/${categoryView.category3Name}`">{{categoryView.category3Name}}</router-link>
        </div>
      </div>
      <!-- 主要内容区域 -->
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          <div class="goodsDetail">
            ...
            <h3 class="news">{{skuInfo.skuDesc}}</h3>
            <div class="priceArea">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="cartWrap">
                <div class="controls">
                  <!-- readonly="readonly" 这里可以有两种做法：
                    第一种input不能输入，只能通过加减修改购买商品数量，且只能在1-5数量中选择
                    第二种input可以输入，但不建议使用，因为商品数量不是能随便购买的，这里如果随便输入，需要控制购买商品数量的最大数量。
                  -->
                  <input
                    autocomplete="off"
                    readonly="readonly"
                    ...
                    @change="handler"
                  />
                  <a href="javascript:" class="plus" @click="skuNum < 5 ? skuNum++ : skuNum">+</a>
...
export default {
  ...
  methods: {
    changeChecked(saleAttrValue, arr) {
      ...
      console.log('选中的版本', saleAttrValue.isChecked)
      arr.forEach((item) => {
        item.isChecked = '0'
      })
      console.log(
        '重置数组中的值为0，arr[0]',
        arr[0].isChecked,
        arr[0].saleAttrValueName
      )
      console.log(
        '重置数组中的值为0，arr[1]',
        arr[1].isChecked,
        arr[1].saleAttrValueName
      )
      ...
      console.log(
        '为选中的值重新赋值给数组,数组中的值arr[0]',
        arr[0].isChecked,
        arr[0].saleAttrValueName
      )
      console.log(
        '为选中的值重新赋值给数组，数组中的值arr[1]',
        arr[1].isChecked,
        arr[1].saleAttrValueName
      )
    },
